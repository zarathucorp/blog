...
preview: https://www.tidyverse.org/images/tidyverse-default.png
categories:
  - lecture
  - tidyverse
  - data.table
...
```{r readfunc, fig.cap="파일 읽기 함수 비교^[https://csgillespie.github.io/efficientR/5-3-importing-data.html]", echo = F}
...
기존의 **data.frame** 외에도 **tbl_df**, **tbl** 와 같은 것들이 추가되어 있다. **tibble**은 **data.frame**보다 좀 더 날 것(?)의 정보를 보여주는데, 범주형 변수를 `factor`가 아닌 `character` 그대로 저장하고 변수명을 그대로 유지하는 것(**data.frame**에서 변수명의 특수문자나 공백은 **.**으로 바뀜)이 가장 큰 특징이다. **tibble**의 추가 내용은 [jumpingrivers 블로그](https://www.jumpingrivers.com/blog/the-trouble-with-tibbles/)^[https://www.jumpingrivers.com/blog/the-trouble-with-tibbles/]를, **[readr](https://readr.tidyverse.org/)**의 다른 함수들은 아래의 [Data Import Cheat Sheet
](https://github.com/rstudio/cheatsheets/raw/master/data-import.pdf)^[https://github.com/rstudio/cheatsheets/raw/master/data-import.pdf] 를 참고하기 바란다.


<embed src="https://drive.google.com/viewerng/
viewer?embedded=true&url=https://github.com/rstudio/cheatsheets/raw/master/data-import.pdf" width="100%" height="600">
...
```{r, echo = F, eval = T}
out <- a %>% filter(Age >= 50) %>% select(-Patient_ID, -STRESS_EXIST) %>% group_by(Sex, Smoking) %>% summarize_all(funs(mean = mean, sd = sd))
paged_table(out)
```
...
```{r, echo = F, eval = T}
out <- a %>% subset(Age >= 50) %>% .[, -c(1, 14)] %>% aggregate(list(Sex = .$Sex, Smoking = .$Smoking), FUN = function(x){c(mean = mean(x), sd = sd(x))})
out
```

위와 같이 기본 함수로도 ` %>% ` 연산자를 이용하여 그럴듯하게 코드를 작성할 수 있으나, **[dplyr](https://dplyr.tidyverse.org/)** 와 비교했을 때 아무래도 가독성이 떨어진다는 점에서 **[dplyr](https://dplyr.tidyverse.org/)**가 유용한 패키지임을 확인할 수 있었다. 전체 **[dplyr](https://dplyr.tidyverse.org/)** 함수들은 아래의 [Data Transformation Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/master/data-transformation.pdf)^[https://github.com/rstudio/cheatsheets/raw/master/data-transformation.pdf]에서 확인할 수 있다. 
...
본 내용에서는 `for`문이나 멀티코어의 사용은 언급하지 않는다. 해당 내용은 [과거 강의](https://jinseob2kim.github.io/radv1.html#faster_for-loop)^[https://jinseob2kim.github.io/radv1.html#faster_for-loop]를 참고하기 바란다. 
