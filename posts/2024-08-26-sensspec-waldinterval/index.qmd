...
어떤 테스트의 민감도와 특이도가 대조 테스트에 비해 떨어지지 않는다는 것을 확인하기 위해서는 비열등성 검정을 사용해야 합니다.
이 때는 `sesp.diff.ci` 함수를 통해 구한 standard error 값과 두 민감도/특이도의 차이를 통해 p value를 계산합니다.
아래 예시에서는 민감도에 대한 비열등성 마진(`sens_margin`)을 5%, 특이도에 대한 비열등성 마진(`spec_margin`)을 10%로 설정했습니다.  
