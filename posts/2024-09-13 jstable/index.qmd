...
당뇨병 환자의 당뇨약 복용 이후 혈당 변화를 예시로 생각해보겠습니다. 이를 일반적인 GLM 모형인 

$$
g(\mu_{i}) = X_{i} \beta
$$

에 적용한다면, 여기서 \( g \)는 링크 함수, \( X \)는 종속 변수를 나타냅니다. 이 모델은 같은 환자에서 반복된 혈당 측정치들이 모두 **독립적**이라는 가정을 필요로 합니다.

그러나, 같은 환자의 경우 혈당 측정치들이 상관관계를 가지기 때문에, 이러한 상관관계를 반영한 모델은 다음과 같이 쓸 수 있습니다:
