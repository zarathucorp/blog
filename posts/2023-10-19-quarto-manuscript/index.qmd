...
YAML header 아래에 본문을 작성합니다. 본문은 Quarto markdown 형식으로 작성하며, 기본적인 사용법은 <a href=“https://quarto.org/docs/authoring/markdown-basics.html”>Quarto markdown 공식 문서</a> 혹은 R markdown 기초를 다루었던 <a href="https://blog.zarathu.com/posts/2019-01-03-rmarkdown/#r-chunk">이전 게시글</a>에서 참고할 수 있습니다. 본 게시글에서는 논문 작성에 필요한 몇 가지 기능을 중점적으로 살펴보겠습니다.
...
원고를 깃허브 페이지를 통해 웹으로 발행하기 위해 `_quarto.yml`파일의 `project: `에 `output-dir: docs` 설정을 추가합니다. 이후 `index.qmd` 파일으로 돌아와 **Render** 버튼을 누르거나 터미널에 `quarto render`을 입력합니다. 모든 변경사항을 깃허브에 commit 후 push 하면, 원고가 깃허브 페이지를 통해 웹으로 발행됩니다. 깃허브의 해당 레포지토리의 **settings > pages**에서 웹페이지 링크를 확인할 수 있습니다.
